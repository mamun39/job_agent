...
- Prompt-driven search does not yet use a selective two-stage enrichment strategy; when Greenhouse or Lever detail enrichment is enabled, detail pages are fetched per discovered job rather than only for listings that already look promising from title-level or listing-level signals.
