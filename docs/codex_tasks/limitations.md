...
- Structured `SearchIntent` and `SearchPlan` models now exist for prompt-derived search workflows, a conservative rule-based prompt parser can extract simple explicit constraints into `SearchIntent`, and a conservative compiler can produce supported-source `SearchPlanQuery` entries for Greenhouse and Lever, but deeper semantic understanding, automatic board-seed selection, and execution wiring are not implemented yet.
